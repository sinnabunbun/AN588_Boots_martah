---
title: "martah_PeerCommentary_vfrench3"
...
*V: You can call them from either the coef() or coefficient() functions which will extract the coefficient estimates from the model object. You can also call them on the summary() of the model object to extract all the estimates for the standard error and the p and t values as well. 
...
I wanna use the beta coef. for parameters so i'm seting up empty holders for the values (*V: YES!) but i'm not sure what it's really asking me to sample?
```{r boots}
slope_sample <- NULL 
...
# **V: a sample size of 30 could be okay but it is right on the verge of the division between using a q or t test so I would bump it up a little more! 
# **V: Also I would sample the number of rows instead of the sequence 1 to 6. This allows all of the variables to be available for sampling (instead of just the first 6 rows) and creates a more general call that could be used on a data set outside of the one we are currently working on. 
...
```
**V: Again just use the coef() function! Also I coerced it into a dataframe so I could more easily subset the coefficients I want and feed them into the NULL variable. 
...
**V: So I did the same thing! My only issue is these will result in numbers that differ from the Std. Err estimate in the model which is confusing me. 
...
ci_intercept
```
**V: Looks good! I used quantile which I think is wrong because it is giving me different values from yours?? 
...
3. I'm not sure when we're asked for the linear regression if we're suppose to do it by hand or graph it bc right now I'm just looking at the summary of lm which gives me the info I need **V: This should be fine! The lm summary gives you alot of information
